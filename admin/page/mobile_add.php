...
<?php
  if(isset($_POST['submit'])){
	...
	if(file_exists("image_upload/new_mobile/" . $_FILES['file']['name'])){
	...
	if(!empty($file_name)){
	  $sql="insert into `new_mobile` (`title`,`photo`) values ('".$title."','".$file_name."')";
	}else{
	  $sql="insert into `new_mobile` (`title`) values ('".$title."')";
	}
	/* echo '<pre>';
	print_r($sql);
	echo '</pre>';
	...
	$rst = mysql_query($sql);
	if($rst){
	  $mess="Mobile Added Successfully !";
	}else{
