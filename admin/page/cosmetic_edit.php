<?php
	if(isset($_GET['id'])){
		$id = $_GET['id'];
		$sql="SELECT * FROM `new_cosmetic` WHERE `id`='".$id."'";
...
<form name="new_mbl" method="post" action="" enctype="multipart/form-data">
  <table class="ltable" border="0" cellpadding="0" cellspacing="0" width="60%" align="center">
	...
	<tr>
	  ...
	  <p><img src="image_upload/new_cosmetic/<?php echo $info['photo']; ?>" height="150" width="150" /></p>
...
<?php
	if (isset($_POST['update'])) {
		...
		if (!empty($_FILES['file']['name']) and file_exists("image_upload/new_cosmetic/" . $_FILES['file']['name'])) {
		...
		if (!empty($file_name)) {
			//unlink($data['file']);
			$sql = "UPDATE  `new_cosmetic` set `title` = '".$title."' , `photo` = '".$file_name."' where id= $id";
		} else {
			$sql = "UPDATE  `new_cosmetic` set `title` = '".$title."' , `photo` ='$info[photo]' where id= $id";
		}
		/* echo '<pre>';
		print_r($sql);
		echo '</pre>';
		...
		if ($rst) {
			...
			unlink("image_upload/new_cosmetic/".$oldfile);
			header('Location:home.php?area=page&&action=edit_del_cosmetic&&msg='.$_SESSION['msg']);
		} else {
